...
export function Hero() {
  ...
  return (
    <section className="border-b border-border-token/70">
      <Container className="grid gap-12 py-20 md:gap-16 md:py-28 lg:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)] lg:items-end">
        <div className="section-grid max-w-3xl gap-8">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <Link href="/#insights" className={buttonClasses({ variant: "gold" })}>
              {t("primaryCta")}
            </Link>
            <Link
              href="/#roadmap"
        ...
        <div className="rounded-[var(--radius-lg)] border border-border-token bg-surface px-6 py-8 md:px-8">
          <div className="section-grid gap-6">
            ...
            <div className="section-grid gap-4">
              {["signal", "capital", "decisions"].map((item) => (
                <div
                  key={item}
                  className="rounded-[var(--radius-md)] border border-border-token bg-background/30 p-4"
                >
